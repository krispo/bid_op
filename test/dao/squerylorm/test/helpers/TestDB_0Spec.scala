...
class TestDB_0Spec extends Specification with AllExpectations {
  ...
  "fill_DB" should {
    ...
    "put 2 Banner belonging to Campaigns" in {
      TestDB_0.creating_and_filling_inMemoryDB() {
        inTransaction {
          ...
          campaign.bannerPhrasesRel.toList.groupBy(_.banner.get.network_banner_id).size must_== (2)
        }
      }
    }
    ...
    "put 2 BannerPhrases belonging to Banner and Phrase" in {
      TestDB_0.creating_and_filling_inMemoryDB() {
        inTransaction {
          import org.squeryl.PrimitiveTypeMode._
